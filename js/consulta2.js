...
$(document).ready(function() {
    ...
    var table = $('#example').DataTable({
        ...
        lengthMenu: [[10, 20, 30, -1], [10, 20, 30, 'Todos']],
              ajax: {
                url: '/SCL/disponibilidade/eventos/dados.php',
                dataSrc: 'data'
                },
                responsive: true,
                orderCellsTop: true,
    ...
    $('#example input').eq(1).css("width","3rem");
    $('#example input').eq(2).css("width","24rem");
    $('#example input').eq(4).css("width","7rem");
    // $('#example input').eq(5).css("width","3rem");
    $('#example input').eq(6).css("width","7rem");
    // $('#example input').eq(7).css("width","4rem");
    $('#example input').eq(8).css("width","7rem");
    ...
    $("#buttonmobilefilter").click(function() {
    var colunavalor = parseInt($("#saidaselect").val());
    var textvalor = $("#textfilter").val();
    var table = $('#example').DataTable();
    
    table.column(colunavalor).search(textvalor).draw();
    });
